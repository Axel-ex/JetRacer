...
class OLEDDisplayNode : public rclcpp::Node
{
    public:
        OLEDDisplayNode() : Node("oled_display_node")
        {
            ...
            uint8_t rc = ssd1306_init(1); // Initialize OLED on I2C node 0
            if (rc != 0)
            {
            ...
            // Set up the OLED display configuration
            rc = ssd1306_oled_default_config(
                32, 128); // Set it to 128x32 resolution
            if (rc != 0)
            {
                RCLCPP_ERROR(this->get_logger(),
